...
- 全局开关键固定为 Home，游戏内也可直接开关
- 使用 Win32 `SendInput` 和扫描码发送按键
- 按住时长固定为 80 毫秒，减少过短按键造成的漏识别
- 开始后自动最小化到后台，停止时自动恢复窗口
- 配置自动保存，并兼容旧版同目录配置
- 每次按键都保证完整的按下时长并成对松开
...
- 如果目标程序以管理员身份运行，KeyLoop 通常也需要以管理员身份运行，否则会受到
  Windows UIPI 限制。
- Home 固定作为开关键，不会出现在普通按键槽的选项中。
- 点击开始后会先最小化，再延迟 200 毫秒开始发键，避免 KeyLoop 窗口占用前台焦点。
